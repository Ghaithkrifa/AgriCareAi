...
def loading___data(**context):
    ...
    with pd.ExcelWriter('weather_data.xlsx', engine='xlsxwriter') as writer:
        df1.to_excel(writer, sheet_name='ForecastingWeather')
        df2.to_excel(writer, sheet_name='ForecastHours')
        df3.to_excel(writer, sheet_name='TodayWeather')
...
with DAG('weather_ETL',
    ...
    Extract_data >>[transform_forecasting_weather,transform__forcast_hours,transform_today_weather]>>loading_data